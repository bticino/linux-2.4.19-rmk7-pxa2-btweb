...
static struct btweb_flavor fltab[] __initdata = {
	...
	{0x5,0x5, BTWEB_PE,    	   "PE",        64, 400, &feat, &init_pe}, /* Full speed !!! */
	...
	{0x7,0x7, BTWEB_H4684_IP,  "H4684_IP",  64, 400, &feat, &init_h4684ip},
        ...
        {0xf,0xf, BTWEB_MH200N,    "MH200N",    64, 200, &feat, &init_pxa255_gw},
#if 0  /* Old Rubini */
...
//	ram_iopage=0xc0000100; /* Start of uboot ATAG : see bdinfo uboot command */
	...
	if ((*ram_iopage=='B')&&(*(ram_iopage+1)=='M')&&(*(ram_iopage+2)=='N')&& \
		...
		id = BTWEB_F453;
                ...
                (*(ram_iopage+5)=='N'))  {
                printk(KERN_INFO "MH200N detected from cmdline: id forced to 0x%x\n",BTWEB_MH200N);
                id = BTWEB_MH200N;
	} else if ((*ram_iopage=='M')&&(*(ram_iopage+1)=='H')&&(*(ram_iopage+2)=='2')&& \
		(*(ram_iopage+3)=='0'))  {
	...
	*memsize = fptr->memsize << 20;
	...
	if (((btweb_globals.flavor!=BTWEB_BMNE500) || \  
		...
		(btweb_globals.flavor!=BTWEB_MH200N) &&  \
	...
	if (fptr->speed == 200) {
		...
		printk(KERN_ALERT "F453 or MH200 or BMNE500 or LGR03617 or MH200N: not fixing GPIO\n");
