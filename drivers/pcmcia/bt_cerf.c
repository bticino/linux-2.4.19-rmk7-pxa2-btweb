/*
 * preso da drivers/pcmcia/sa1100_cerf.c
 * E.Parmeggiani
...
#else
#define pcmcia_debug 3     /* gcc will remove all the debug code for us */
#endif
...
/*	printk( "bt_cerf: irq richiesto %d %d \n", i, PCMCIA_GPIO_CF_IRQ_EDGE ); */
	...
	printk( "bt_cerf: requested irq %d %d \n", i, irqs[i].irq );
...
{
	int i = CERF_SOCKET;
//	la funzione vien chiamata 1 volta al secondo
	...
	state_array->state[i].bvd1   = cerf_pcmcia_level_bvd1();	// !!!parm -- nn ci sono 'ste funzioni
	state_array->state[i].bvd2   = cerf_pcmcia_level_bvd2();	// !!!parm -- nn ci sono 'ste funzioni
...
// socket_init:		cerf_pcmcia_socket_init,		// sono da fare ??
// socket_suspend:	cerf_pcmcia_socket_suspend,		// sono da fare ??
};
