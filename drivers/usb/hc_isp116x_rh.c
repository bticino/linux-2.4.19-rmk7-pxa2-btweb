...
static int rh_send_irq (hci_t * hci, void * rh_data, int rh_len)
{
	...
	num_ports = hc116x_read_reg32 (hci, HcRhDescriptorA) & RH_A_NDP;
	
	*(__u8 *) data = (hc116x_read_reg32 (hci, HcRhStatus) & (RH_HS_LPSC | RH_HS_OCIC))
		? 1: 0;
	ret = *(__u8 *) data;

	for ( i = 0; i < num_ports; i++) {
		*(__u8 *) (data + (i + 1) / 8) |= 
			((hc116x_read_reg32 (hci, HcRhPortStatus + i) &
...
#define WR_RH_STAT(x) 		hc116x_write_reg32 (hci, (x), HcRhStatus)
#define WR_RH_PORTSTAT(x) 	hc116x_write_reg32 (hci, (x), HcRhPortStatus + wIndex - 1)
#define RD_RH_STAT		hc116x_read_reg32 (hci, HcRhStatus)
#define RD_RH_PORTSTAT		hc116x_read_reg32 (hci, HcRhPortStatus + wIndex - 1)
...
int rh_submit_urb (urb_t * urb)
{
	...
	*/
		...
		case RH_GET_DESCRIPTOR | RH_CLASS:
		    {
			    __u32 temp = hc116x_read_reg32 (hci, HcRhDescriptorA);
			    ...
			    temp = hc116x_read_reg32 (hci, HcRhDescriptorB);
