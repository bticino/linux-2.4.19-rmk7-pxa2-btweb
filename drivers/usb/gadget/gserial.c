...
static int gs_write(struct tty_struct *tty, int unused,
...
static int __init gs_module_init(void)
{
	...
	gs_tty_driver->table = gs_tty;
	gs_tty_driver->termios = gs_termios;
	gs_tty_driver->termios_locked = gs_termios_locked;
	gs_tty_driver->refcount = &gs_refcount; 
...
#else
	gs_tty_driver->open = gs_open;
	gs_tty_driver->close =		gs_close;
	gs_tty_driver->write =		gs_write;
	gs_tty_driver->put_char =		gs_put_char;
	gs_tty_driver->flush_chars =		gs_flush_chars;
	gs_tty_driver->write_room =		gs_write_room;
	gs_tty_driver->ioctl =		gs_ioctl;
	gs_tty_driver->set_termios =		gs_set_termios;
	gs_tty_driver->throttle =		gs_throttle;
	gs_tty_driver->unthrottle =		gs_unthrottle;
	gs_tty_driver->break_ctl =		gs_break;
	gs_tty_driver->chars_in_buffer =	gs_chars_in_buffer;
#endif
...
static void gs_notify_complete(struct usb_ep *ep, struct usb_request *req)
{
	printk("gs_notify_complete\n");
}
 ...
 * gs_write
 */
static int gs_write(struct tty_struct *tty, int unused, const unsigned char *buf,
		     int count)
{
	unsigned long flags;
	struct gs_port *port = tty->driver_data;
...
gs_debug_level(3, "gs_send: len=%d, 0x%2.2x 0x%2.2x 0x%2.2x ...\n", len, *((unsigned char *)req->buf), *((unsigned char *)req->buf+1), *((unsigned char *)req->buf+2));
			gs_debug("req_e %p\n",req_entry);
			gs_debug("re_e %p\n",&req_entry->re_entry);
			gs_debug("req_e %p\n",req_entry->re_entry.next);
			gs_debug("req_e %p\n",req_entry->re_entry.prev);
	...
	const struct usb_ctrlrequest *ctrl)
{
	...
	gs_debug("gs_setup: bRequestType=%x\n",ctrl->bRequestType); 
	...
	case USB_REQ_SET_INTERFACE:
		...
		if (dev->dev_config != GS_BULK_CONFIG_ID
			...
			if (dev->dev_notify_ep) {
				...
				printk(KERN_INFO "gs_setup_standard: enabling dev_notify_ep\n"); //!!!raf
...
static void gs_send_line_state (struct gs_dev *dev)
{
	int ret,len;
	unsigned long flags;
	struct usb_ep *ep;
	...
	if (dev == NULL) {
		printk(KERN_ERR "gs_send_line_state: NULL device pointer\n");
		return -ENODEV;
	}
	...
	while(!list_empty(&dev->dev_req_list)) {
		...
		gs_debug ("trying\n");
		event = req->buf;
		event->bmRequestType = 0x21;
		event->bNotificationType = USB_CDC_REQ_SET_CONTROL_LINE_STATE;
		event->wValue = __constant_cpu_to_le16 (0x03);	/* connected */
		event->wIndex = __constant_cpu_to_le16 (1);
		event->wLength = 0;
		req->length = sizeof *event;
		req->context = dev;

		len = usb_ep_queue (ep, req, GFP_ATOMIC);
		gs_debug ("usb_ep_queue result = %d\n", len);
		
		if (len < 0)
			gs_debug ("gs_send_line_state: status buf queue --> %d\n", value);
		if (len > 0) {
gs_debug_level(3, "gs_send_line_state: len=%d, 0x%2.2x 0x%2.2x 0x%2.2x ...\n", len, *((unsigned char *)req->buf), *((unsigned char *)req->buf+1), *((unsigned char *)req->buf+2));
			list_del(&req_entry->re_entry);
			req->length = len;
			if ((ret=usb_ep_queue(ep, req, GFP_ATOMIC))) {
				printk(KERN_ERR
				"gs_send_line_state: cannot queue read request, ret=%d\n",
					ret);
				break;
			}
		} else {
	...
	const struct usb_ctrlrequest *ctrl)
{
	...
	case USB_CDC_REQ_SET_CONTROL_LINE_STATE:
		gs_debug("gs_setup_class: USB_CDC_REQ_SET_CONTROL_LINE_STATE\n"); 
#if 0 //CONFIG_USB_CDC_LINE_STATE
		ret = min(wLength,(u16)sizeof(struct usb_cdc_line_coding));
		gs_debug("gs_setup: class request, type=%02x, request=%02x, value=%04x, index=%04x, length=%d\n",
			ctrl->bRequestType, ctrl->bRequest,
			wValue, wIndex, wLength);
		gs_send_line_state(dev);
		gs_debug("gs_setup exiting\n");
#else
		ret = 0;
#endif
...
#if 0
	{
		struct usb_request *req;
		static int init=0;
		static unsigned char buf[] = {0xa1, 0x20, 0, 0, 0, 0, 0x2, 0, 0x3, 0};

		req = usb_ep_alloc_request(dev->dev_notify_ep, GFP_ATOMIC);
		req->buf = buf;
		req->length = 10;
		req->complete = gs_notify_complete;
		usb_ep_queue(dev->dev_notify_ep, req, GFP_ATOMIC);
