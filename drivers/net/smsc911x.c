...
static int __init link_mode_set(char *str)
{
	int par;
...
__setup("smc_mode=", link_mode_set);
...
static irqreturn_t Smsc911x_ISR(int irq,void *dev_id,struct pt_regs *regs);
...
int __init Smsc911x_init(struct net_device *dev)
{
	DWORD dwLanBase=0UL;
	DWORD dwIdRev=0UL;
	DWORD dwFpgaRev=0UL;
	...
	privateData->dwLanBase= lan_base;
	if (!privateData->dwLanBase)
	  return -ENOMEM;
...
static int Smsc911x_open(struct net_device *dev)
{
	...
	irq = IRQ_GPIO(22);
	set_GPIO_IRQ_edge(22, GPIO_FALLING_EDGE);
...
static int Smsc911x_hard_start_xmit(
...
static struct net_device_stats * Smsc911x_get_stats(struct net_device * const dev)
{
#if 0
  WORD dummy;
  WORD ddummy;
...
#if 0
	SMSC_TRACE("------GET_STATS/DUMP DEI REGISTRI-----");
	dummy = Lan_GetRegDW(ID_REV);
	SMSC_TRACE("ID_REV=%x",dummy);
	dummy = Lan_GetRegDW(INT_CFG);
	SMSC_TRACE("INT_CFG=%x",dummy);
	dummy = Lan_GetRegDW(INT_STS);
	SMSC_TRACE("INT_STS=%x",dummy);
	dummy = Lan_GetRegDW(INT_EN);
	SMSC_TRACE("INT_EN=%x",dummy);
	dummy = Lan_GetRegDW(BYTE_TEST);
	SMSC_TRACE("BYTE_TEST=%x",dummy);
	dummy = Lan_GetRegDW(FIFO_INT);
	SMSC_TRACE("FIFO_INT=%x",dummy);
	dummy = Lan_GetRegDW(RX_CFG);
	SMSC_TRACE("RX_CFG=%x",dummy);
	dummy = Lan_GetRegDW(TX_CFG);
	SMSC_TRACE("TX_CFG=%x",dummy);
	dummy = Lan_GetRegDW(HW_CFG);
	SMSC_TRACE("HW_CFG=%x",dummy);
	dummy = Lan_GetRegDW(RX_FIFO_INF);
	SMSC_TRACE("RX_FIFO_INF=%x",dummy);
	dummy = Lan_GetRegDW(TX_FIFO_INF);
	SMSC_TRACE("TX_FIFO_INF=%x",dummy);
	dummy = Lan_GetRegDW(PMT_CTRL);
	SMSC_TRACE("PMT_CTRL=%x",dummy);
	dummy = Lan_GetRegDW(GPIO_CFG);
	SMSC_TRACE("GPIO_CFG=%x",dummy);
	dummy = Lan_GetRegDW(GPT_CFG);
	SMSC_TRACE("GPT_CFG=%x",dummy);
	dummy = Lan_GetRegDW(GPT_CNT);
	SMSC_TRACE("GPT_CNT=%x",dummy);
	dummy = Lan_GetRegDW(ENDIAN);
	SMSC_TRACE("ENDIAN=%x",dummy);
#endif
...
#if 0
	// Salvo lo stato degli interrupt  
	//	spin_lock_irqsave(&(privateData->IntEnableLock),dwIntFlags1);
	dwIntEn=Lan_GetRegDW(INT_EN);
	Lan_SetRegDW(INT_EN,0);
	//        spin_unlock_irqrestore(&(privateData->IntEnableLock),dwIntFlags1);
#endif
...
DONE:
	...
	/* Check that the input is still low */
	if (!(GPLR(22) & GPIO_bit(22))) {
		/*
		 * The manual says 1us is the minimum to detect an edge,
		 * so turn irqen off for more than 1us (raf, ARub)
		 */
		spin_lock_irqsave(&(privateData->IntEnableLock),dwIntFlags1);
		dwIntEn=Lan_GetRegDW(INT_EN);
		Lan_SetRegDW(INT_EN,0);
		udelay(2);
		Lan_SetRegDW(INT_EN,dwIntEn);
		spin_unlock_irqrestore(&(privateData->IntEnableLock),dwIntFlags1);
	}
	...
	DWORD dwLinkRequest)
{
	...
	/* 
	   ...
	   Some unuseful reads
	   and phy reset forcing

	mytemp=Lan_GetRegDW(PMT_CTRL);	
	SMSC_TRACE("PMT_CTRL=%x",mytemp);
	udelay(1000);
	//SMSC_TRACE("PMT_CTRL write");
	//Lan_SetRegDW(PMT_CTRL,0x400);	

	mytemp=Lan_GetRegDW(PMT_CTRL);	
	SMSC_TRACE("PMT_CTRL=%x",mytemp);

	udelay(1000); //wait reset done
	mytemp=Lan_GetRegDW(PMT_CTRL);	
	SMSC_TRACE("PMT_CTRL=%x",mytemp);

	dwPhyAddr=0xFFFFFFFFUL;
	...
	*/
	if(dwPhyAddr!=0xFFFFFFFFUL) {
		...
		default:
			SMSC_WARNING("Default: External Phy is not supported");
			SMSC_WARNING("  using internal phy instead");
			goto USE_INTERNAL_PHY;
		}
	} else {
...
DONE:
#if 0
	SMSC_TRACE("Phy_GetRegW dwAddr=%x,i=%x,res=%x",(unsigned int)dwAddr,i,result); //!!!raf
#endif
	return result;
}
...
DONE:
#if 0
	SMSC_TRACE("gres=%x",(unsigned int)result); //!!!raf
#endif
	return result;
}
...
#if 0
	SMSC_TRACE("soffs=%x",(unsigned int)((dwRegOffset & 0x000000FFUL) | MAC_CSR_CMD_CSR_BUSY_) );   //!!!raf
#endif
	...
	struct sk_buff *skb)
{
	...
	if(privateData->dwTxDmaCh>=TRANSFER_PIO) 
	{
		...
		dev_kfree_skb(skb);
		do {
			unsigned int res = Lan_GetRegDW(TX_STATUS_FIFO);
			prevtag = res>>16;
			if (res & 0xffff)
				printk(KERN_DEBUG "packet %5i, error 0x%04x\n",
				       res & 0xffff);
		} while (tag - prevtag > 1);
	} 
	else 
...
//Rx_ReceiverOff, and Rx_ReceiverOn use a reference counter
//  because they are used in both the Rx code and the link management count
void Rx_ReceiverOn(PPRIVATE_DATA privateData)
{
	DWORD dwIntFlags=0;
	VL_KEY keyCode=Vl_WaitForLock(&(privateData->MacPhyLock),&dwIntFlags);
	if(privateData->dwRxOffCount>0) {
...
//		SMSC_TRACE("Multicast: HASHH=0x%08lX,HASHL=0x%08lX",dwHashH,dwHashL);
		Mac_SetRegDW(privateData,HASHH,dwHashH,keyCode);
		Mac_SetRegDW(privateData,HASHL,dwHashL,keyCode);
		Mac_SetRegDW(privateData,MAC_CR,dwMacCr,keyCode);
	} 
	else 
	{
		...
		dwMacCr&=(~MAC_CR_MCPAS_);
		dwMacCr&=(~MAC_CR_HPFILT_);
		Mac_SetRegDW(privateData,HASHH,0,keyCode);
		Mac_SetRegDW(privateData,HASHL,0,keyCode);
		Mac_SetRegDW(privateData,MAC_CR,dwMacCr,keyCode);
