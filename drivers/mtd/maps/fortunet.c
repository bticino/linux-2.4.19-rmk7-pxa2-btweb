...
struct map_region
{
	...
	int			num_probes;
	char			probe_name[MAX_NAME_SIZE*2];
...
static int __init MTD_New_Region(char *line)
{
	...
	map_regions[params[1]].num_probes = 1;
	...
	if(params[0]>5)
	{
		...
		if(*liner)
		{
			liner = get_string_option(map_regions[params[1]].probe_name,
				sizeof(map_regions[params[1]].probe_name),liner);
			map_regions[params[1]].num_probes = 1;
			pos = strlen(map_regions[params[1]].probe_name)+1;
			while((*liner)&&(pos+2<sizeof(map_regions[params[1]].probe_name)))
			{
				liner = get_string_option(
					map_regions[params[1]].probe_name+pos,
					sizeof(map_regions[params[1]].probe_name)-pos,liner);
				pos += strlen(map_regions[params[1]].probe_name+pos)+1;
				map_regions[params[1]].num_probes++;
			}
		}
	}
	return 1;
	...
	for(iy=ix=0;ix<MAX_NUM_REGIONS;ix++)
	{
		...
		if(map_regions_set[ix])
		{
			...
			printk(KERN_NOTICE MTD_FORTUNET_PK "%s flash is veritualy at: %x\n",
				...
				map_regions[ix].map_info.map_priv_1);
			map_regions[ix].mymtd = NULL;
			for(ipos=0,iz=0;(!map_regions[ix].mymtd)&&
				(iz<map_regions[ix].num_probes);iz++,
				ipos += strlen(map_regions[ix].probe_name+ipos)+1)
			{
				if(iz)
					printk(KERN_NOTICE MTD_FORTUNET_PK
						"Trying alternate probe %s for %s\n",
						map_regions[ix].probe_name+ipos,
						map_regions[ix].map_info.name);
				map_regions[ix].mymtd = do_map_probe(
					map_regions[ix].probe_name+ipos,
					&map_regions[ix].map_info);
			}
			if(map_regions[ix].altbuswidth!=
				map_regions[ix].map_info.buswidth)
			{
				...
				{
					printk(KERN_NOTICE MTD_FORTUNET_PK
						"Trying alternate sized(%d) probe %s for %s\n",
						map_regions[ix].altbuswidth,
						map_regions[ix].probe_name+ipos,
						map_regions[ix].map_info.name);
					map_regions[ix].mymtd = do_map_probe(
